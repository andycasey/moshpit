...
if __name__ == '__main__':
    ...
    # Perturb positions
    init_positions = true_state.positions + jr.normal(keys[3], (n_stars, 2)) * 1
    ...
    # Test both fitting methods
    import time

    # Method 1: Block-coordinate descent (original)
    print("\n" + "="*70)
    print("METHOD 1: Block-Coordinate Descent (fit_model_jit)")
    print("="*70)

    t0 = time.time()
    final_state_block, log_likes_block = fit_model_jit(
        data, pixel_coords, init_state, field_center,
        n_iter=n_iter, fit_sky=True, track_likelihood=True
    )
    final_state_block.positions[0].block_until_ready()
    t1 = time.time()
    print(f"Time: {t1-t0:.3f}s")

    pos_errors_block = jnp.sqrt(jnp.sum((final_state_block.positions - true_state.positions)**2, axis=1))
    amp_errors_block = jnp.abs(final_state_block.amplitudes - true_state.amplitudes) / true_state.amplitudes * 100

    print(f"\nPosition RMSE: {jnp.sqrt(jnp.mean(pos_errors_block**2)):.4f}")
    print(f"Position median error: {jnp.median(pos_errors_block):.4f}")
    print(f"Amplitude mean error: {jnp.mean(amp_errors_block):.1f}%")
    print(f"Amplitude median error: {jnp.median(amp_errors_block):.1f}%")
    print(f"Final log L: {log_likes_block[-1]:.2f}")

    # Method 2: Joint least squares with line search (new)
    print("\n" + "="*70)
    print("METHOD 2: Joint Least Squares with Line Search (fit_model_jit_joint_linesearch)")
    print("="*70)
    ...
    print("COMPARISON")
    print("="*70)
    print(f"ΔLog L: {log_likes_joint[-1] - log_likes_block[-1]:.2f} (joint - block)")
    print(f"ΔPosition RMSE: {jnp.sqrt(jnp.mean(pos_errors_joint**2)) - jnp.sqrt(jnp.mean(pos_errors_block**2)):.4f} (joint - block)")
    print(f"ΔAmplitude mean error: {jnp.mean(amp_errors_joint) - jnp.mean(amp_errors_block):.1f}% (joint - block)")
    ...
    print("\nPlots for Block-Coordinate Descent:")
    plot_results(
        data=data,
        true_model=true_model,
        fitted_model=eval_model(pixel_coords, final_state_block, field_center),
        true_state=true_state,
        initial_state=init_state,
        final_state=final_state_block,
        image_shape=(n_pix_side, n_pix_side),
        log_likes=[float(ll) for ll in log_likes_block],
        prefix="block"
    )
    ...
    ax = axes[0]
    ax.plot([float(ll) for ll in log_likes_block], 'b-o', markersize=3, label='Block-Coordinate', alpha=0.7)
    ax.plot([float(ll) for ll in log_likes_joint], 'r-s', markersize=3, label='Joint LS', alpha=0.7)
    ax.set_xlabel('Iteration')
    ax.set_ylabel('Log-likelihood')
    ax.set_title('Full Convergence Comparison')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax = axes[1]
    zoom_start = max(0, n_iter // 2)
    zoom_iters = list(range(zoom_start, n_iter + 1))
    ax.plot(zoom_iters, [float(ll) for ll in log_likes_block[zoom_start:]], 'b-o', markersize=4, label='Block-Coordinate', alpha=0.7)
    ax.plot(zoom_iters, [float(ll) for ll in log_likes_joint[zoom_start:]], 'r-s', markersize=4, label='Joint LS', alpha=0.7)
    ax.set_xlabel('Iteration')
    ax.set_ylabel('Log-likelihood')
    ax.set_title(f'Zoom: Last {n_iter + 1 - zoom_start} Iterations')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    print("  - block_*.png (block-coordinate descent results)")
    print("  - joint_*.png (joint least squares results)")
    print("  - comparison_convergence.png (side-by-side convergence)")
